...
class AuthController extends Controller
{
    ...
    public function otpVerification(Request $request)
    {
        ...
        try {
            ...
            // update OTP to used (mark the OTP record as IsActive = true)
            $this->bk_db->table('portaluserotphistory')
                ->where('Id', $otp_exists->Id)  // Use OTP record ID from join result
                ->update(['IsActive' => true]);
            ...
            $errorMessage = config('app.debug') ? $th->getMessage() : 'Error verifying OTP';
